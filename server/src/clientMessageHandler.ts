...
function handleWebviewReady(send: WsSend, ctx: ClientMessageContext): void {
  ...
  send({
    ...
    readingTools: [...(runtime?.providers.readingTools ?? [])],
    subagentToolNames: [...(runtime?.providers.subagentToolNames ?? [])],
  });
  ...
  const identities: Record<
    number,
    { providerId: string; agentKey: string; displayName: string; role: string; model?: string }
  > = {};
  for (const [id, agent] of store) {
    ...
    if (agent.providerId && agent.agentKey && agent.displayName && agent.role) {
      identities[id] = {
