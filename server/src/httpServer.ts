...
export async function createHttpServer(options: HttpServerOptions): Promise<HttpServerHandle> {
  ...
  registerTasksRoute(app, options);
...
function registerHookRoute(app: FastifyInstance, options: HttpServerOptions): void {
  app.post<{
    ...
    async (request, reply) => {
      ...
      const registry = options.runtime?.providers ?? createProviderRegistry();
      if (!registry.get(providerId)) {
        return reply.code(400).send({ error: `Unknown provider: ${providerId}` });
      }

      const sessionId = (event.session_id as string) ?? '';
      const eventKind = (event.hook_event_name as string) ?? '';
      writeAuditLog('hook_received', {
        provider: providerId,
        sessionId,
        event: eventKind,
      });
...
function registerRosterRoute(app: FastifyInstance, options: HttpServerOptions): void {
  app.get('/api/roster', async (_request, reply) => {
    const roster = options.runtime?.roster ?? loadAgentRoster();
    reply.header('Content-Type', 'application/json');
    return roster.entries;
  });
}

function registerTasksRoute(app: FastifyInstance, _options: HttpServerOptions): void {
  app.post<{
    Body: { agentId: string; task: string };
  }>('/api/tasks/assign', async (request, _reply) => {
    const { agentId, task } = request.body;
    console.log(`[Pixel Agents] Task assigned to agent ${agentId}: ${task}`);
    writeAuditLog('task_assigned', {
      agentId,
      task,
    });
...
function hookBearerAuth() {
  return async (request: FastifyRequest, reply: FastifyReply) => {
    const hookToken = process.env.PIXEL_OFFICE_HOOK_TOKEN;
    if (!hookToken) {
      return; // Skip auth if env var is not set
    }
    const auth = request.headers.authorization ?? '';
