...
import { loadAgentRoster } from './roster.js';
...
export class PixelAgentsServer {
  ...
  async start(options?: {
    ...
    if (
      ...
      (await isServerHealthy(existing.port))
    ) {
    ...
    if (existing) this.deleteStaleServerJson();
  ...
  /** Remove a discovery file that failed the live health probe. PID reuse is
   * common across container restarts (the Node process is usually PID 1), so a
   * PID check alone cannot establish that the recorded server still exists. */
  private deleteStaleServerJson(): void {
    try {
      const filePath = this.getServerJsonPath();
      if (fs.existsSync(filePath)) fs.unlinkSync(filePath);
    } catch {
      // writeServerJson will surface a useful error if replacement also fails.
    }
  }
}
...
/** Confirm that the process recorded in server.json actually owns a live Pixel
 * Agents HTTP server. This prevents stale PID collisions after Docker restarts. */
function isServerHealthy(port: number): Promise<boolean> {
  return new Promise((resolve) => {
    const request = http.get(
      { hostname: '127.0.0.1', port, path: '/api/health', timeout: 750 },
      (response) => {
        response.resume();
