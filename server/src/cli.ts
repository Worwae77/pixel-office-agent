...
function parseArgs(argv: string[]): CliArgs {
  const args: CliArgs = { port: 3100, host: '127.0.0.1', manageHooks: true };
  for (let i = 0; i < argv.length; i++) {
    if ((argv[i] === '--port' || argv[i] === '-p') && argv[i + 1]) {
      ...
      args.host = argv[i + 1];
      i++;
    } else if (argv[i] === '--no-manage-hooks') {
      args.manageHooks = false;
    } else if (argv[i] === '--help') {
...
Options:
  ...
  --no-manage-hooks     Receive hooks without modifying host CLI hook settings
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const onSetHooksEnabled = async (enabled: boolean): Promise<void> => {
      ...
      if (!args.manageHooks) {
        console.log('[Pixel Agents] Hook management disabled; settings were not modified');
        return;
      }
      if (enabled) {
    ...
    if (runtime.hooksEnabled.current && args.manageHooks) {
      try {
