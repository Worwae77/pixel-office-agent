...
describe('PixelAgentsServer', () => {
  ...
  let origHookToken: string | undefined;

  beforeEach(() => {
    origHookToken = process.env.PIXEL_OFFICE_HOOK_TOKEN;
    delete process.env.PIXEL_OFFICE_HOOK_TOKEN;
  ...
  afterEach(() => {
    server?.stop();
    if (origHookToken !== undefined) {
      process.env.PIXEL_OFFICE_HOOK_TOKEN = origHookToken;
    } else {
      delete process.env.PIXEL_OFFICE_HOOK_TOKEN;
    }
    try {
  ...
  it('does not reuse a stale discovery file when the PID was recycled', async () => {
    fs.writeFileSync(
      serverJsonPath,
      JSON.stringify({
        port: 1,
    ...
    expect(config.port).toBeGreaterThan(1);
    const response = await fetch(`http://127.0.0.1:${config.port}/api/health`);
    expect(response.status).toBe(200);
  });

  // 3. Hook endpoint skips auth by default (when PIXEL_OFFICE_HOOK_TOKEN is unset)
  it('hook endpoint skips auth when env token is unset', async () => {
    const config = await server.start();
  ...
  // 4. Hook endpoint enforces auth when PIXEL_OFFICE_HOOK_TOKEN is set
  it('hook endpoint enforces auth when env token is set', async () => {
    process.env.PIXEL_OFFICE_HOOK_TOKEN = 'secure-token';
    const config = await server.start();

    // Without auth header -> 401
    const resNoAuth = await fetch(`http://127.0.0.1:${config.port}/api/hooks/claude`, {
      method: 'POST',
      body: '{}',
    });
    ...
    const resBadAuth = await fetch(`http://127.0.0.1:${config.port}/api/hooks/claude`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
    ...
    const resOk = await fetch(`http://127.0.0.1:${config.port}/api/hooks/claude`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
