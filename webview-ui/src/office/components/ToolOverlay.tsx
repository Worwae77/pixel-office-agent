  ...
  return (
    <>
      {allIds.map((id) => {
        ...
        const identityLabel = ch.displayName
          ? `${ch.displayName}${ch.role ? ` · ${ch.role}` : ''}`
          : null;
        const primaryLabel = teamRoleLabel ?? identityLabel;
        const totalTokens = ch.inputTokens + ch.outputTokens;
        const tokenRatio = totalTokens / MAX_CONTEXT_TOKENS;
        const hasExtraLines = !!(ch.folderName || primaryLabel);
